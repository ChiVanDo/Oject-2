...
function clicktaosp(){ // khi click vao ham nay se lay thong tin cua input
    ...
    var nodekieu = document.getElementById('kieu');
    var kieu = nodekieu.value;
    nodekieu.value='';
    ...
    // var sp = taodoituongsp(hinhanh1,hinhanh2,hinhanh3,hinhanh4,hinhanh5, ten, giagoc, giamgia, null , kieu);
    // // đưa sp vào ds
    // dssp.push(sp);
    // // tiếp theo lưu trữ dssv vào local
    // var json_dssp = JSON.stringify(dssp);
...
function taodoituongsp(hinhanh1,hinhanh2,hinhanh3,hinhanh4,hinhanh5, ten ,giagoc, giamgia, masp, kieu){
    var sp = new Object();
...
function chuyenDanhsachdoituongsanphamthanhHTML_TOP(dssp){
    var html = '<div class="nhieu_sp">';   
    for(var i=0; i < dssp.length ; i++){
        if(dssp[i] != null && dssp[i].kieu != 2 && dssp[i].kieu != 3){
            var sanpham= dssp[i];
            var htmlsp = chuyendoituongspThanhHTML(sanpham);
            html += htmlsp;
        }
...
// hien thi quan
function chuyenDanhsachdoituongsanphamthanhHTML_quan(dssp){
    var html = '<div class="nhieu_sp">';
    
    for(var i=0; i < dssp.length ; i++){
        if(dssp[i] != null && dssp[i].kieu == 2){
            var sanpham= dssp[i];
            var htmlsp = chuyendoituongspThanhHTML(sanpham);
            html += htmlsp;
        }   
...
function chuyenDanhsachdoituongsanphamthanhHTML_phukien(dssp){
    var html = '<div class="nhieu_sp">';
    for(var i=0; i < dssp.length ; i++){
        if(dssp[i] != null && dssp[i].kieu == 3){
            var sanpham= dssp[i];
            var htmlsp = chuyendoituongspThanhHTML(sanpham);
            html += htmlsp;
        }
...
function chitiet_sphtml(sanpham){
    ...
    html+=         '<div class="dat_hang"><a href="dat_hang.html"><button>Đặt Hàng</button></a></div>';
    html+=        '</div>';
    html+=     '</div>';
...
function chi_tiet_sp(msp){
    ...
    for(var i = 0 ; i < dssp.length ; i++){
        ...
        if(sp_canlay != null && sp_canlay.masp == id_sp){
            var html = '<div class="nhieu_sp">';
    ...
    document.getElementById('chuyentrang').style.display='none';
